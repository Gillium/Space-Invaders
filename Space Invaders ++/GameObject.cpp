...
void GameObject::update(float fps)
{
	if (m_Active && m_HasUpdateComponent)
	{
		...
		{
			shared_ptr<UpdateComponent> tempUpdate = static_cast<UpdateComponent>(m_Components[i]);
...
shared_ptr<Component> GameObject::getComponentByTypeAndSpecificType(string type, string specificType)
{
	...
	cout << "GameObject.cpp::getComponentByTypdeAndSpecificType-" << "COMPONENT NOT FOUND ERROR!" << endl;
#endif
